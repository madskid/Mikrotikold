...
def lic_gen_ros(software_id, private_key: bytes, version=6, feature=22):
    assert (isinstance(private_key, bytes))
    if isinstance(software_id, str):
        software_id = mikro_softwareid_decode(software_id)
    lic = software_id.to_bytes(6, 'little')
    varb7 = version  # RouterOS Version
    varb8 = feature  # Features
...
if __name__ == '__main__':
    ...
    licgenros_parser.add_argument('-v', '--version', type=int, default=6, help='RouterOS Version (default: 6)')
    licgenros_parser.add_argument('-f', '--feature', type=int, default=22, help='Feature Level (default: 22)')
    ...
    if args.command == 'genkey':
        ...
        print(lic_gen_ros(args.software_id, bytes.fromhex(args.private_key), args.version, args.feature))
